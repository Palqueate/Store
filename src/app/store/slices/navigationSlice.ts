...
import { palcoOccupancy, withCart, seatEventAvailable } from '@/modules/palcos/domain/availability'
...
export const createNavigationSlice = (set, get) => ({
  ...
  selectPalco: (id) => {
    ...
    set({ pId: id, eventId: eid, occurrenceId: get()._firstOcc(eid), seats: [], parkSel: 0, food: [], snacksTarget: 'draft', fromEvent: false })
  },
  openDetail: (id) => { get().selectPalco(id); get().go('detail') },
  openDetailEvent: (pid, eid, occId) => { set({ pId: pid, eventId: eid, occurrenceId: occId || get()._firstOcc(eid), seats: [], parkSel: 0, food: [], snacksTarget: 'draft', fromEvent: true }); get().go('detail') },
  ...
  detailVM: () => {
    ...
    // Ocupación por función: lo sembrado + lo que el carrito ya reservó para este
    // palco, para que dos líneas del carrito no pisen la misma butaca.
    const occ = withCart(palcoOccupancy(p), get().cart, p.id)
    // Estado de cada butaca para la función elegida.
    const seats: any[] = []
    for (let i = 1; i <= p.seats; i++) {
      ((n) => {
        const free = seatEventAvailable(occ, n, s.occurrenceId)
        const st = !free ? 'taken' : (s.seats.indexOf(n) >= 0 ? 'sel' : 'free')
    ...
    const qty = s.seats.length
    let total = p.modes.seatEvent.price * qty
    ...
    total += parkTotal
    return { p, markers, seats, events, total, qty, parkAvail, parkPrice, parkSel, parkTotal, price: p.modes.seatEvent.price }
  },
})
