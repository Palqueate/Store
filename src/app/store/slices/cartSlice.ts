...
export const createCartSlice = (set, get) => ({
  ...
  addToCart: () => {
    ...
    // palco. Validación autoritativa: no se agrega una butaca ya tomada.
    ...
    const ev = get().events.find((e) => e.id === s.eventId); const occu = ev ? eventOccurrence(ev, s.occurrenceId) : null
    const item: any = { uid: 'c' + Date.now() + Math.floor(Math.random() * 999), palcoId: p.id, palcoTitle: p.title, stadium: p.stadium }
    item.mode = 'seatEvent'; item.modeLabel = 'Asiento · por evento'; item.seats = s.seats.slice().sort((a, b) => a - b); item.eventId = s.eventId; item.occurrenceId = s.occurrenceId; item.eventLabel = ev ? (ev.comp + (ev.round ? (' · ' + ev.round) : '')) : ''; item.eventOpp = ev ? ev.opp : ''; item.term = occu ? (occu.day + ' ' + occu.month + ' · ' + occu.time + ' hs') : ''; item.qty = s.seats.length; item.price = p.modes.seatEvent.price * s.seats.length
  ...
  cartItemVM: (it) => {
    const seatsText = 'Asiento' + (it.seats.length > 1 ? 's' : '') + ' ' + it.seats.join(' · ')
    const meta = it.eventLabel
    const tag = 'ASIENTO'
    ...
    const qtyNote = (seatQty > 1) ? (' · ' + get().money(seatUnit) + ' × ' + seatQty + ' asientos') : ''
