...
export default function StadiumMap({ stadium = 'gpc', name, mapImage, markers = [], interactive = false, onPick }: StadiumMapProps) {
  ...
  const built = (markers || []).map((m) => {
    ...
    const z = kind === 'sel' ? 6 : (kind === 'open' ? 5 : 3)
    const clk = (typeof m.click === 'function')
      ? (e: React.MouseEvent) => { if (e && e.stopPropagation) e.stopPropagation(); m.click!() }
      : undefined
    const wrap = 'position:absolute; left:' + m.x + '%; top:' + m.y + '%; transform:translate(-50%,-50%); z-index:' + z + ';' + (clk ? ' cursor:pointer;' : '')
    let dot: string
    if (kind === 'sel') {
      ...
      dot = 'width:25px; height:25px; border-radius:7px; background:var(--primary,#C9A24B); border:2px solid var(--background,#0E1116); box-shadow:0 6px 16px -5px rgba(0,0,0,.55); display:grid; place-items:center; color:var(--primary-foreground,#1A1407); font-family:Space Mono; font-weight:700; font-size:11px;'
    } else {
      dot = 'width:17px; height:17px; border-radius:5px; background:var(--card,#171B22); border:1.5px dashed var(--subtle-foreground,#6B7480); opacity:.5;'
    }
    return { wrapStyle: wrap, dotStyle: dot, label: (kind === 'sel' && m.label) ? m.label : '', badge: (m.badge != null ? String(m.badge) : ''), onClick: clk }
  })
  ...
  return (
    <div ref={elRef} onClick={onField} style={css(containerStyle)}>
      ...
      {/* palco markers */}
      {built.map((m, i) => (
        <div key={i} onClick={m.onClick} style={css(m.wrapStyle)}>
          {m.label ? (
