...
export function usePublishVM(): any {
  ...
  if (w) {
    ...
    wiz = {
      ...
      stadium: w.stadium, stadiumName: STADIUMS[w.stadium].name, stadiumShape: (STADIUMS[w.stadium] || {}).shape || 'rect', x: w.x, y: w.y, title: w.title, seats: String(w.seats), parkN: String(w.parkN), parkHas: w.parkHas,
      images: w.images || [],
