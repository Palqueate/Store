...
export function useAdminVM(): any {
  ...
  var adminStadiums = STAD_LIST.map(function (st) { return { id: st.id, name: st.name, short: st.short, city: st.city || '—', address: st.address || '—', capacity: st.capacity ? st.capacity.toLocaleString('es-UY') : '—', year: st.year || '—', surface: st.surface || '—', levels: String(st.levels || '—'), roof: (st.roof ? 'Techado' : 'Abierto'), palcos: String(ALL_PALCOS.filter(function (p) { return p.stadium === st.id }).length), events: String(EVENTS.filter(function (e) { return e.stadium === st.id }).length) } })
