...
export function useDetailVM(): any {
  ...
  if (dvm) {
    ...
    det.host = dp.host; det.seatsN = dp.seats; det.stadium = dp.stadium; det.stadiumShape = (STADIUMS[dp.stadium] || {}).shape || 'rect'; det.stadiumMap = (STADIUMS[dp.stadium] || {}).mapImage || ''; det.markers = dvm.markers; det.photos = dp.photos; det.images = dp.images || []
